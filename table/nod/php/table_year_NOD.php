...
$stid = oci_parse($conn,"select ID, NAME, NAME_ORG, NAME_SLUZH, AUTORS, ID_NOMINAC, ZNACHIM, ID_STRUC, ID_FILE, case when  VNEDR_D is null then to_char(' ') else to_char(VNEDR_D) end as VNEDR_D, VNEDR_M, VYBOR_NOD FROM MI_LIST WHERE ID_NOD = :id_nod and DATE_REG BETWEEN TO_DATE (:date_start, 'DD.MM.YYYY') AND TO_DATE (:date_end, 'DD.MM.YYYY') - 1 order by DATE_REG DESC");
